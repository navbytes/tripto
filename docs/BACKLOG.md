...
- **A2 — Enable the email-import pipeline** (MX / routing to
  `plans.tripto.navbytes.io`). ✅ **LIVE since 2026-07-11.** MX + catch-all active on `plans.tripto.navbytes.io`, worker deployed, shared secret set on both sides, Cloudflare credits loaded. Go-live verified end-to-end per Handover documentation (2026-07-10) and re-verified read-only 2026-07-14 (web/email-worker/RUNBOOK.md + scripts/check-golive.sh). Remaining §A work = EI-4 remainder (reprocess path, rate-limit tests, unverified-sender UX).
